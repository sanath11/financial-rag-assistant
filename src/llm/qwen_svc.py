...
Qwen via OpenRouter integration for financial RAG answers.
OpenRouter free tier: varies by model; some endpoints are sponsored.
...
def _get_headers():
    ...
    return {
        ...
        "HTTP-Referer": "https://github.com/financial-rag-assistant",
        "X-Title": "Financial RAG Assistant",
    }
...
def generate_answer(
    ...
    """Generate a financial answer using Qwen via OpenRouter given retrieved context."""
    ...
    try:
        ...
        if "quota" in error_msg.lower() or "rate" in error_msg.lower():
            return "❌ OpenRouter API rate limit exceeded. Check your account."
        elif "api" in error_msg.lower() and "key" in error_msg.lower():
            return "❌ Invalid OpenRouter API key. Check your .env file."
        else:
...
def generate_suggested_questions(tickers: list[str], years: list[str]) -> list[str]:
    """Ask Qwen to generate 5 relevant questions based on available tickers and years."""
...
2. Compare TSLA and AAPL revenue growth strategies"""
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as resp:
            data = json.loads(resp.read().decode("utf-8"))
            lines = data["choices"][0]["message"]["content"].strip().splitlines()
            questions = []
            for line in lines:
                line = line.strip()
...
def _fallback_questions(tickers: list[str]) -> list[str]:
    """Generic questions used if Qwen call fails."""
